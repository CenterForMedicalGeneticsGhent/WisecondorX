...
- **Data Science:** NumPy, SciPy, Pandas, Scikit-learn, PyArrow
...
1. **Convert:** BAM/CRAM files are converted into binned read counts stored in compressed NumPy (`.npz`) or parquet (`.parquet`) files.
2. **Newref:** Multiple `.npz` or `.parquet` files from control samples are combined to create a reference `.npz` file. This involves PCA and gender modeling.
...
- `src/wisecondorx/predict.py`: Functional logic for the predict stage.
- `src/wisecondorx/convert.py`: Functional logic for the convert stage.
- `src/wisecondorx/newref.py`: Functional logic for creating reference files.
...
- **Surgical Updates:** When fixing bugs, prioritize small, targeted changes and adding a corresponding test case.
- **Documentation:** Update this document and README if you make changes that affect the overall architecture or development workflow.